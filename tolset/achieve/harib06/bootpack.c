...
void HariMain(void)
{
	...
	memman_init(memman);
	memman_free(memman, 0x00001000, 0x0009e000); /* 0x00001000 - 0x0009efff の間に空きメモリであることを設定 */
	memman_free(memman, 0x00400000, memtotal - 0x00400000); /* 0x00400000から最終番地まで空きメモリとする。 */
...
{
	char flg486 = 0; // CPUアーキテクチャが486以降であるかのフラグ。
	unsigned int eflg, cr0, i;

	/* 386か、486以降なのかの確認 */
	eflg = io_load_eflags();
	eflg |= EFLAGS_AC_BIT; /* AC-bit = 1 にしておいて*/
	io_store_eflags(eflg);
	eflg = io_load_eflags();
	if ((eflg & EFLAGS_AC_BIT) != 0) { /* 386ではAC=1にしても自動で0に戻ってしまう */
		flg486 = 1; /*　これで486以降であるをチャックできる。　*/
	}
	eflg &= ~EFLAGS_AC_BIT; /* AC-bit = 0 */
	io_store_eflags(eflg); /* eflgビットを0に戻す */
	...
	if (flg486 != 0) {
		cr0 = load_cr0();
		cr0 &= ~CR0_CACHE_DISABLE; /* キャッシュ許可に戻す */
		store_cr0(cr0);
	}
