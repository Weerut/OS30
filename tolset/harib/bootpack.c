	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (key_win->flags == 0) {	/* ウィンドウが閉じられた */
				key_win = shtctl->sheets[shtctl->top - 1];//Activeシートを一番上のシートにする
...
int keywin_off(struct SHEET *key_win, struct SHEET *sht_win, int cur_c, int cur_x)
{
	change_wtitle8(key_win, 0); //指定したシートをInactiveにする。
	if (key_win == sht_win) { // 指定シートがAtiveウィンドウズの場合
		cur_c = -1; /* カーソルを消す */
		boxfill8(sht_win->buf, sht_win->bxsize, COL8_FFFFFF, cur_x, 28, cur_x + 7, 43);//タイトルバーをInactive色に
	} else {
		if ((key_win->flags & 0x20) != 0) {//Consoleが必要なウィンドウズの場合
			fifo32_put(&key_win->task->fifo, 3); /* コンソールのカーソルOFF　（タスクにくっついたFifoを通して）*/
		}
	}
	return cur_c;
}
...
int keywin_on(struct SHEET *key_win, struct SHEET *sht_win, int cur_c)
{
	change_wtitle8(key_win, 1);//指定したシートをInactiveにする。
	if (key_win == sht_win) {// 指定シートがAtiveウィンドウズの場合
